...
const Admin = () => {
  ...
  const itemList = users.map((item) => (
    <li key={item.name}>
      ...
      Sharp Eye: {((999 - item.time) / 1000) * 100} %
      <br />
      Focus : {((999 - item.time1) / 1000) * 100} % 
      <br />
      Critical Thinking: {((999 - item.time2) / 1000)* 100} %
      <br />
      Observation:{((999 - item.time3) / 1000) * 100} %
      <br /> <br />
      <br />
    </li>
  ));
  ...
  return (
    <>
      ...
      >
        ...
        >
          <i> Welcome! Agent Fury </i>
        </span>
        <div
          className="asd"
          style={{
            backgroundColor: "rgba(220, 88, 88, 0.71)",
            color: "black",
            padding: "30px",
            borderRadius: "20px",
          }}
